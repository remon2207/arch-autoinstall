...
check_variables() {
    if [ ${2} != "intel" ] -a [ ${2} != "amd" ]; then
        echo "Missing argument or misspelled..."
        return 1
    elif [ ${3} != "xfce" ] -a [ ${3} != "gnome" ] -a [ ${3} != "kde" ]; then
        echo "Missing argument or misspelled..."
        return 1
    elif [ ${4} != "nvidia" ] -a [ ${4} != "amd" ] -a [ ${4} != "intel" ]; then
        echo "Missing argument or misspelled..."
        return 1
    elif [ ${9} != "yes" ] -a [ ${9} != "no-exclude-efi" ] -a [ ${9} != "no-root-only" ] -a [ ${9} != "skip" ]; then
        echo "Missing argument or misspelled..."
        return 1
    elif [ ${10} != "systemd-boot" ] -a [ ${10} != "grub" ]; then
        echo "Missing argument or misspelled..."
        return 1
    elif [ ${11} != "static-ip" ] -a [ ${11} != "dhcp" ]; then
        echo "Missing argument or misspelled..."
        return 1
    fi
}
